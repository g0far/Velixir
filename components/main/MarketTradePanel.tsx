...
import { MarketToken, SWAP_HISTORY } from "@/constants/market";
...
import { executeTreasurySwap, TreasuryUnavailableError } from "@/lib/swap";
...
const MarketTradePanel = ({ tokens, selectedToken, onTokenChange }: MarketTradePanelProps) => {
  ...
  const globalAddress = useWalletStore((s) => s.address);
  ...
  const logSwap = (hash: string) => {
    ...
    SWAP_HISTORY.unshift(tx);
    if (account) recordSwap(account, tx);
  };
  ...
  const handleSwap = async () => {
    ...
    try {
      ...
      // Real settlement: the treasury co-signs an atomic transfer so the user's
      ...
      try {
        ...
        if (swapErr instanceof TreasuryUnavailableError) {
          // No treasury configured — fall back to a memo-recorded tx so the swap
          // still lands on Devnet (balances unchanged until treasury is set).
          ...
          if (status === "success") logSwap(hash);
        } else {
          throw swapErr;
        }
      }
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#1a1a3e]">
        ...
        {account ? (
          ...
          <span className="text-[10px] text-gray-500 uppercase tracking-wide">Solana Devnet</span>
        )}
      </div>
      ...
      <div className="flex-1 p-4 flex flex-col gap-1.5 overflow-y-auto">
        ...
        <div className="bg-[#0a0a24] border border-[#1a1a3e] rounded-xl p-3 focus-within:border-cyan-500/40 transition-colors">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] text-gray-400 font-mono">Balance: {getBalance(fromToken.symbol)}</span>
          </div>
        ...
        <div className="bg-[#0a0a24] border border-[#1a1a3e] rounded-xl p-3">
          <div className="flex items-center justify-between mb-2">
            ...
            <span className="text-[10px] text-gray-400 font-mono">Balance: {getBalance(toToken.symbol)}</span>
          </div>
          <div className="flex items-center gap-2">
